...
const RENDER_API_URL = "https://explore-armenia-api.onrender.com";
...
async function generateTour() {
  ...
  try {
    ...
    document.getElementById('result_info').innerText =
      `Обработано дней: ${data.days_processed} | Найдено мест: ${data.places_found}`;
    
    // Предупреждения о ненайденных местах
    if (data.places_not_found && data.places_not_found.length > 0) {
      document.getElementById('not_found_warning').classList.remove('hidden');
      document.getElementById('not_found_list').innerText =
        data.places_not_found.join(', ');
    }

  } catch (e) {
    // Render бесплатный засыпает — первый запрос может занять ~30 сек
    if (e.message.includes('fetch') || e.message.includes('network')) {
      showError("Сервер просыпается (~30 сек). Попробуйте ещё раз через 30 секунд.");
    } else {
      showError(e.message);
    }
  } finally {
